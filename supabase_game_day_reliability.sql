create or replace function public.add_team_touch(
  p_team_id uuid,
  p_player_id text
)
returns jsonb
...
begin
  ...
  tracker = jsonb_set(tracker, '{counts}', coalesce(tracker->'counts', '{}'::jsonb), true);
  tracker = jsonb_set(tracker, '{history}', coalesce(tracker->'history', '[]'::jsonb), true);

  current_count = coalesce((tracker #>> array['counts', p_player_id])::integer, 0) + 1;
  tracker = jsonb_set(tracker, array['counts', p_player_id], to_jsonb(current_count), true);
  tracker = jsonb_set(
    tracker,
    '{history}',
    coalesce(tracker->'history', '[]'::jsonb) ||
